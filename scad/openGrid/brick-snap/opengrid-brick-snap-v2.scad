/*
===========================================================
   OpenGrid Brick Snap – Flush & Seamless Tiles
===========================================================

📦 Purpose
Bring standard building-brick compatibility to your OpenGrid projects.

This OpenSCAD script generates two fully parametric tiles:
  • **Flush Tile** — Matches the height of an OpenGrid board.
    Perfect for pixel art, decorative plates, or flat color fills.
  • **Seamless Tile** — Adds a raised deck that extends halfway into
    the board seam, creating half-studs that combine into full studs
    between neighboring tiles. Ideal for spanning plates and bricks
    across multiple OpenGrid cells.

🧰 How to Use
1. Choose a variant:
      snap_variant = "flush";     // flat, within one cell
      snap_variant = "seamless";  // raised, bridges across cells

2. Adjust key parameters near the top:
      stud_rescale     = 1.03;   // printer fit
      seam_clear_total = 0.20;   // total gap for seamless edges
      deck_thick       = 2.6;    // tile deck thickness
      chamfer_enable   = true;   // micro-chamfer under lip

🖨️ Print Recommendations
- Layer height: 0.20 mm (tested) or 0.16 mm for fine detail
- No supports; print with studs up
- 3 walls / perimeters, full cooling
- Chamfer helps reduce underside stringing
- PLA or PETG; use stud_rescale 1.03–1.04 for PLA

📏 Compatibility
- Standard building-brick stud pitch: 8.0 mm; diameter 4.8 mm
- OpenGrid board spacing: 28 mm centers
- Seamless variant aligns half-studs for cross-tile clutch

🧩 Credits
- David D — OpenGrid concept  
- metasyntactic — OpenSCAD implementation of OpenGrid snap  
- Katie + Hands On Katie community  
- cfinke — Original `stud()` and `rounded_stud_top()` modules  
- ejwill — Project author and maintainer  
  https://github.com/ejwill/ejwill-scad
...
- Portions of the stud geometry adapted from LEGO.scad (cfinke, **CC BY 3.0**)  
...
⚠️ Disclaimer
This project is not endorsed by or affiliated with the LEGO Group.
"LEGO" is a trademark of the LEGO Group, which does not sponsor,
authorize, or endorse this design.
===========================================================
*/
...
// False is recommended unless you are trying to hold a lot of weight
snap_directional= false; // [true, false]
// Seamless allows bridging across seams, flush is only built within snap area
snap_variant    = "seamless"; // [flush, seamless]
...
/* [Seamless Deck Options] */
// Clearance deck seams
seam_clear_total = 0.20; // [0.00:0.01:1.00]
// How thick the deck on top of the snap is
...
$fn = 100;
snap_lite       = (board_type == "lite") ? true : false;
//Tile
snap_top        = 24.8;
pitch_board     = 28.0;
seam_total 		= pitch_board - snap_top;
overhang   		= max(0, seam_total/2 - seam_clear_total/2);
edge_pos   		= snap_top/2 + overhang;
deck_w     		= snap_top + 2*overhang;
deck_h     		= snap_top + 2*overhang;
corner_r        = 3.0;
// micro chamfer
chamfer_z       = 0.12;
chamfer_xy      = 0.20;
...
module clipped_stud_field(snap_w, snap_h, pitch, offx, offy){
  r=adj_stud_diameter/2;
  min_x=-snap_w/2 - r; max_x=snap_w/2 + r;
  min_y=-snap_h/2 - r; max_y=snap_h/2 + r;
  ...
  intersection(){
    ...
    translate([0,0,stud_height/2]) cube([snap_w, snap_h, stud_height+0.3], center=true);
  }
}
...
module snap_deck_midseam_halves_indexed(col=0,row=0){
