...
clamp_width = 35;
clamp_depth = 60;
clamp_thickness = 15; // minimum 5, do not exceed clamp length
corner_shape = 0; //[0:Round,1:Square]
rounding = 2; // radius of ball that makes round edges
roundedness = 8; //[4:4:64] // how smoothly rounded or polygonal the rounding is (24 is plenty)
bolt_hole_space_factor = 4.3; // adjusts how tight the threads mesh
/*[bolt]*/
knob_length = 15;
knob_diameter = 24;
knob_knurled_finish = 400; // a lower number makes finer points
screw_diameter = 16;
screw_length_reduction = 0; // keeps the bolt from getting in the way but limits it to clamping objects bigger this amount
assembled_screw_position = 0; // shows the preview opened by this much
/*[press]*/
press_diameter = 24;
press_thickness = 5; //[2:5] // 2 is zero thickness for press diameter
ball_and_socket_clearance = 0.3; // 0.3mm fit snug and 0.6mm was too loose on my prints
...
if (show_assembled)
    rotate([0, 0, -90]) {
        ...
        translate([-knob_length - radius * thickness - rounding - screw_pos, hole_shift, clamp_width / 2]) {
            rotate([0, 90, 0]) bolt();
            translate([knob_length + screw_length + 5 + 8, 0, 0])
                rotate([0, -90, 0]) press();
        }
    }
else {
...
module acorner(tr) {
    ...
    intersection() {
        difference() {
            cylinder(r = tr, h = width, $fn = 64);
            translate([0, -rounding - thickness / 2, -0.1])
                cylinder(r = thickness * (radius - 1), h = width + .2, $fn = 32);
...
module hex_screw(od, st, lf0, lt, rs, cs, df, hg, ntl, ntd) {
    let(ntr = od / 2 - (st / 2) * cos(lf0) / sin(lf0)) {
        union() {
            hex_head(hg, df);
